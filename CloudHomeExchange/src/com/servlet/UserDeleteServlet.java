...
public class UserDeleteServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(uid != null) {
			...
			if(flag == true) {
				response.getWriter().append("<script language='javascript'>alert('delete user success.');</script>");
				response.sendRedirect("background/user.jsp" );
			}else {
				response.getWriter().append("<script language='javascript'>alert('failed to delete user');"
						+ "history.back();</script>");
			}
		}else {
